...
export default function CategoriesSection({
  ...
  const maxTracks = Math.max(...categories.map((cat) => cat.tracks), 1);
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.from({ length: 6 }).map((_, i) => (
            <Card
              key={i}
              className="animate-pulse h-44 group cursor-pointer overflow-hidden bg-white/5 backdrop-blur-xl border border-white/10 hover:bg-white/10 hover:border-white/20 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 rounded-3xl p-6 sm:p-8"
            />
          ))}
        </div>
      </section>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="w-full h-1 bg-gray-700 rounded-full overflow-hidden">
                      <div
                        ...
                        style={{
                          width: `${(category.tracks / maxTracks) * 100}%`,
                        }}
                      />
                    </div>
                  </div>
